...
public class DrivetrainSubsystem extends SubsystemBase {
    ...
    public void loop() {
        ...
        if (USING_TAGS) {
            ...
            tagPose = frontLocalizer.getPoseEstimate();
        }
    }
    ...
    public ArrayList<AprilTagDetection> getUsedTags() {
        if (USING_TAGS) {
            return frontLocalizer.getUsedTags();
        } else {
            return new ArrayList<>();
        }
    }

    public AprilTagLocalizer2d getFrontLocalizer() {
        return frontLocalizer;
    }

    public boolean isDetected() {
        if (USING_TAGS) {
            return frontLocalizer.isDetected();
        } else {
            return false;
        }
    }
